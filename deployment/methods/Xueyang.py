...
@DP_OBJECT.register
def deployment_one_road(map: LoaclMap, gap_pole, cost_pole, cost_cameras, cameras_info, road_list):
    for road_id in tqdm(map.get_lines()):
        road: Line = map.get_line(road_id)
        if road.get_tag("highway") not in road_list:
            continue
